...
const fieldClass = 'rounded-md border border-black/10 bg-white px-4 py-3 text-sm font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-current/35 focus:border-[var(--slot4-teal)]'
    ...
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--slot4-page-bg)] px-4 py-16 text-[var(--slot4-page-text)] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-lg bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="editable-dots flex h-full min-h-72 items-center justify-center rounded-lg bg-[var(--slot4-dark-bg)] text-white">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-md bg-[var(--slot4-teal)] px-6 py-3 text-sm font-black text-white">Login <ArrowRight className="h-4 w-4" /></Link>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-lg bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] backdrop-blur lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside className="editable-animate">
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-md border p-4 text-left transition ${active ? 'border-[var(--slot4-teal)] bg-[var(--slot4-teal)] text-white' : 'border-black/10 bg-[var(--slot4-gray)] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-lg bg-[var(--slot4-page-bg)] p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-md bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.16em]">{session.name}</span>
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-md bg-[var(--slot4-dark-bg)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5 hover:bg-[var(--slot4-accent)]">
