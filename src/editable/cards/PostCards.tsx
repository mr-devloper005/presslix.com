...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} style={{ '--editable-index': index } as CSSProperties} className={`group editable-card ${dc.layout.minRailCard} block p-5`}>
      ...
      <h3 className="mt-4 line-clamp-3 text-xl font-black leading-[1.08] tracking-[-.03em]">{post.title}</h3>
      <p className="mt-4 line-clamp-4 text-sm leading-7 text-[var(--slot4-muted-text)]">{getEditableExcerpt(post, 150)}</p>
      <span className="mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-[var(--slot4-teal)]">Read update <ArrowRight className="h-4 w-4" /></span>
    </Link>
  )
}
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group grid min-w-0 grid-cols-[46px_1fr] gap-4 border-t border-current/15 py-5 first:border-t-0">
      <span className="text-3xl font-black leading-none text-[var(--slot4-accent)]">{String(index + 1).padStart(2, '0')}</span>
      <div className="min-w-0">
        <p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] opacity-55"><Clock3 className="h-3 w-3" /> {getEditableCategory(post)}</p>
